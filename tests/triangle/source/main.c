...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "Software keyboard demo - by fincs\n");
	fprintf(stderr, "A: text input\n");
	fprintf(stderr, "B: numpad input\n");
	fprintf(stderr, "X: western input\n");
	fprintf(stderr, "Y: filtered input + HOME/POWER/reset handling\n");
	fprintf(stderr, "START: exit\n");
	...
	while (aptMainLoop())
	{
		...
		if (kDown & KEY_A)
		{
            ...
            printString("Opening text input...\n");
			button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
            printString("Closing text input...\n");
		}
		...
		if (didit)
		{
			if (button != SWKBD_BUTTON_NONE)
			{
				fprintf(stderr, "You pressed button %d\n", button);
				fprintf(stderr, "Text: %s\n", mybuf);
			} else
				fprintf(stderr, "swkbd event: %d\n", swkbdGetResult(&swkbd));
		}
